...
crime2 <- read_xlsx("crime.xlsx") %>% 
  rename("crime_rate" = crmrte,
         ...
         "wage_sta" = wsta,
         "wage_loc" = wloc) %>% 
  mutate(arrest_prob = ifelse(arrest_prob > 100, NA, arrest_prob),
         convict_prob = ifelse(convict_prob > 100, NA, convict_prob))
...
crime2 %>%
  ...
  annotate("text", x = 49, y = 11.5, label = "Mean Arrest Percent: 29.52%")
```
